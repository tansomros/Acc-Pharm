...
Public Class ReportGPP
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            ...
            Select Case Request("s")
                Case "gpp"
                    pnView.Visible = True
                    lblReportTitle.Text = "รายงานสรุปผลการประเมิน GPP"
                Case "fda"
                    pnView.Visible = False
                    cmdView.Visible = False
                    lblReportTitle.Text = "รายงานส่งมอบ GPP "
                Case Else
                    pnView.Visible = False
                    cmdView.Visible = False
                    lblReportTitle.Text = "รายงานคะแนน GPP รายข้อ"
            End Select
    ...
    Private Sub LoadData()
        If Convert.ToInt32(Request.Cookies("ROLE_ID").Value) >= 4 And Convert.ToInt32(Request.Cookies("ROLE_ID").Value) <= 5 Then
            dtGPP = ctlR.RPT_GPP_GetByPPHSearch(ddlYear.SelectedValue, ddlProvinceGroup.SelectedValue, ddlProvince.SelectedValue, Request.Cookies("UserID").Value, txtSearch.Text)
        ...
        Else
            dtGPP = ctlR.RPT_GPP_GetByPPHSearch(ddlYear.SelectedValue, ddlProvinceGroup.SelectedValue, ddlProvince.SelectedValue, "0", txtSearch.Text)
        End If
    End Sub
    ...
    Private Sub cmdExport_Click(sender As Object, e As EventArgs) Handles cmdExport.Click
        Select Case Convert.ToInt32(Request.Cookies("ROLE_ID").Value)
            Case 4
                ctlR.GEN_GPPReportForPPH(ddlYear.SelectedValue, ddlProvinceGroup.SelectedValue, ddlProvince.SelectedValue, Request.Cookies("UserID").Value, txtSearch.Text, Request.Cookies("UserID").Value)
            Case 5
                ctlR.GEN_GPPReportForAdminPPH(ddlYear.SelectedValue, ddlProvinceGroup.SelectedValue, ddlProvince.SelectedValue, txtSearch.Text, Request.Cookies("UserID").Value)
            Case Else
                ctlR.GEN_GPPReportForPPH(ddlYear.SelectedValue, ddlProvinceGroup.SelectedValue, ddlProvince.SelectedValue, "0", txtSearch.Text, Request.Cookies("UserID").Value)
        End Select
        'LoadData()
