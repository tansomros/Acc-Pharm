...
Public Class Allocate
    ...
    Private Sub LoadLocationNoAllocateToGrid()
        If StrNull2Zero(ddlUser.SelectedValue) <> 0 Then
            If txtSearchLocation.Text <> "" Then
                dt = ctlL.Location_GetNoAllocateSearch(ddlYear.SelectedValue, ddlProvince.SelectedValue, StrNull2Zero(ddlUser.SelectedValue), optCond.SelectedValue, txtSearchLocation.Text)
            Else
                dt = ctlL.Location_GetNoAllocate(ddlYear.SelectedValue, ddlProvince.SelectedValue, StrNull2Zero(ddlUser.SelectedValue), optCond.SelectedValue)
            End If
            If dt.Rows.Count > 0 Then
                With grdData
                    .Visible = True
    ...
    Private Sub LoadSupervisorToDDL()
        dt = ctlU.Supervisor_GetByProvince(ddlProvince.SelectedValue)
        'If dt.Rows.Count > 0 Then
        With ddlUser
                .DataSource = dt
    ...
    Private Sub LoadLocationAllocateToGrid()
        If StrNull2Zero(ddlUser.SelectedValue) <> 0 Then
            If txtSearchAllocate.Text <> "" Then
                dt = ctlL.LocationAllocate_GetByUserSearch(StrNull2Zero(ddlYear.SelectedValue), StrNull2Zero(ddlUser.SelectedValue), txtSearchAllocate.Text)
            Else
                dt = ctlL.LocationAllocate_GetByUser(StrNull2Zero(ddlYear.SelectedValue), StrNull2Zero(ddlUser.SelectedValue))
            End If
        ...
        For i = 0 To grdData.Rows.Count - 1
            With grdData
                ...
                If chkS.Checked Then
                    item = ctlL.LocationAllocate_Add(StrNull2Zero(ddlYear.SelectedValue) - 543, ddlUser.SelectedValue, .Rows(i).Cells(1).Text, Request.Cookies("UserID").Value)

                    ctlU.User_GenLogfile(Request.Cookies("UserID").Value, ACTTYPE_ADD, "LocationAllocate", "เพิ่ม ร้านยาให้ผู้ตรวจประเมิน:UserID=" & ddlUser.SelectedValue & ">>LID=" & .Rows(i).Cells(1).Text, "", Environment.MachineName, GetIPAddress())
                End If
            End With
        Next
        LoadLocationAllocateToGrid()
            ...
            Select Case ButtonPressed.ID
                Case "imgDel"
                    If ctlL.LocationAllocate_Delete(e.CommandArgument) Then
                        ctlU.User_GenLogfile(Request.Cookies("UserID").Value, ACTTYPE_DEL, "LocationAllocate_DeleteByUID", "ลบ ร้านยาที่จัดสรรให้ผู้ตรวจประเมิน:" & ddlUser.SelectedValue & ">>" & e.CommandArgument, "", Environment.MachineName, GetIPAddress())
    ...
    Protected Sub lnkSubmitAdd_Click(sender As Object, e As EventArgs) Handles lnkSubmitAdd.Click
        ...
        dt = ctlL.Location_GetNoAllocate(ddlYear.SelectedValue, ddlProvince.SelectedValue, ddlUser.SelectedValue, optCond.SelectedValue)
        If dt.Rows.Count > 0 Then
            For i = 0 To dt.Rows.Count - 1
                ctlL.LocationAllocate_Add(StrNull2Zero(ddlYear.SelectedValue) - 543, StrNull2Zero(ddlUser.SelectedValue), dt.Rows(i)("LocationUID"), Request.Cookies("UserID").Value)
            Next
        End If

        ctlU.User_GenLogfile(Request.Cookies("UserID").Value, ACTTYPE_ADD, "LocationAllocate", "เพิ่ม ร้านยาให้ผู้ตรวจประเมิน แบบทั้งหมด:UserID=" & ddlUser.SelectedItem.Text, "", Environment.MachineName, GetIPAddress())
