...
Public Class ReportAssessment
    ...
    Protected Sub cmdView_Click(sender As Object, e As EventArgs) Handles cmdView.Click
        ...
        If Convert.ToInt32(Request.Cookies("ROLE_ID").Value) = 2 Then
            If Request("s") = "A1" Then
                dtR = ctlRpt.RPT_Assessment_GetScore(Bdate, Edate, ddlType.SelectedValue, ddlStatus.SelectedValue, ddlLocation.SelectedValue, StrNull2Zero(Request.Cookies("UserID").Value))
            Else
                dtR = ctlRpt.RPT_Assessment_GetSWOT(Bdate, Edate, ddlType.SelectedValue, ddlStatus.SelectedValue, ddlLocation.SelectedValue, StrNull2Zero(Request.Cookies("UserID").Value))
            End If
        Else
    ...
    Protected Sub cmdExport_Click(sender As Object, e As EventArgs) Handles cmdExport.Click
        ...
        pnData.Visible = False
        If Convert.ToInt32(Request.Cookies("ROLE_ID").Value) = 2 Then
            ScriptManager.RegisterClientScriptBlock(Me, Me.GetType(), "Report", "window.open('ReportViewer?R=" & Request("s") & "S&ST=" & ddlStatus.SelectedValue & "&LID=" & ddlLocation.SelectedValue & "&TYPE=" & ddlType.SelectedValue & "&BDT=" & Bdate & "&EDT=" & Edate & "&RPTTYPE=EXCEL','_blank');", True)
        Else
            ScriptManager.RegisterClientScriptBlock(Me, Me.GetType(), "Report", "window.open('ReportViewer?R=" & Request("s") & "&ST=" & ddlStatus.SelectedValue & "&LID=" & ddlLocation.SelectedValue & "&TYPE=" & ddlType.SelectedValue & "&BDT=" & Bdate & "&EDT=" & Edate & "&RPTTYPE=EXCEL','_blank');", True)
        End If
